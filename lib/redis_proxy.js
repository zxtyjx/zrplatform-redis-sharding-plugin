...
class RedisProxy extends BasicProxy {
  ...
  async call(callee, args) {
    // exec
    if (callee === 'exec') throw new Error(`请使用execAsync命令处理事务,multi.${callee} is not a function, use multi.execAsync instead.`)
    // discard
    ...
    // 事务校验
    if (callee === 'execAsync' && !this.isMulti) throw new Error(`提交事务请先使用multi命令开启事务.`)
    if (callee === 'execAsync' && this.multis.length === 0) throw new Error(`提交事务请先添加需要执行的Redis命令.`)
